...
Naslednji zemljevid pa nam z razliko od prejšnjega pove bolj koristno informacijo o številu diplomantov, ki se prikazuje kot delež diplomantov glede na število prebivalcev v določeni regiji.

```{r zemljevid2, echo=FALSE, message=FALSE, warning=FALSE,fig.align='center', fig.cap='Povprečno število diplomantov po regijah v zadnjih desetih letih'}
 zemljevid2
```
...
Spodnji tortni diagram prikazuje razmerje diplomantov v preučevanih desetih letih, ki diplomira na rednem študiju v primerjavi s tistimi, ki diplomirajo na izrednem študiju. Iz podatkov je razvidno, da je število diplomantov na izredne študiju precej manj v primerjavi na rednem.

```{r tortnidiagram, echo=FALSE, fig.align='center'}
tortnidiagram
```
...
Spodnji graf nam prikazuje kako se razmerje med številom dijakov in številom prebivalcev v Sloveniji spreminja, kot lahko vidimo, je trend trenutno padajoč, kar pomeni, da je kljub višji narodnosti, dijakov vedno manj. To pomeni, da je trenutno v Sloveniji več otrok nižje starosti in več ljudi, ki so že naredili srednjo šolo. Pričakujemo lahko, da bo trend kmalu naraščajoč.

```{r graf12, echo=FALSE, fig.align='center'}
graf12
```

Naslednji graf pa nam prikazuje razmerje med številom diplomantov in številom prebivalcev v Sloveniji. Kot lahko opazimo je ta trend veliko bolj nenihajoč kot pri prejšnjem grafu. Razberemo lahko, da v Sloveniji letno diplomira približno 20 tisoš študentov.
