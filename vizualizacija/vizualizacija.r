...
graf_povprecja_po_regijah <- ggplot(povprecje.dijakovindiplomantov.po.regijah, aes(x=regija, y=povprecje, fill=kategorija)) + geom_col(position = 'dodge')  + 
  coord_flip() + labs(x = "Regija", y = "Povprečno število", title = "Povprečno število dijakov in diplomantov na leto v \nposameznih statističnih regijah") +
  scale_fill_discrete(name = "Kategorija", labels = c("število dijakov", "število diplomantov")) + scale_fill_brewer()
...
graf5 <- ggplot(podatki5, aes(x=izobrazevanje, y=povprecje, fill = spol)) + geom_col(position = 'dodge')  + 
  ...
  scale_fill_discrete(name = "Spol", labels = c("moški", "ženski")) + scale_fill_brewer()
...
zemljevid2 <- tm_shape(merge(zemljevida, povprecjedipl, by.x="NAME_1", by.y="regija" )) + 
  tm_polygons("povprecje",title="Delež",palette="Purples")+ tm_style("grey") + tm_layout(main.title="Povprečni delež diplomantov po regijah v zadnjih desetih \nletih glede na število prebivalcev v posamezni regiji") + tm_text(text='NAME_1', size=0.6)
